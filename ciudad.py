...
def get_model_state(model):
    ...
    model_state = {
        ...
        "estaciones": [
            (estacion * 15, 12) for estacion in model.ubicaciones_estaciones
        ],  # arrojja las posicones (y, x)
        "pasajeros": [
            {
...
class Ciudad(Model):
    def __init__(self, ubicaciones_estaciones, numero_personas, tiempos):
        ...
        self.ubicaciones_estaciones = [u[0] for u in ubicaciones_estaciones]
        self.numero_personas = numero_personas
        self.schedule = RandomActivation(self)
        self.estaciones = []

        self.chance_ubicaciones_estaciones = []

        for i, v in ubicaciones_estaciones:
            for _ in range(v):
                self.chance_ubicaciones_estaciones.append(i)
        print(self.chance_ubicaciones_estaciones)
        ...
        self.ids.append(possible_id)
        self.tren = tren.Tren(
            possible_id, self, 0, 0
        )  # El tren tendrá un id y una posición inicial
        ...
        for i in range(0, numero_personas):
            ...
            while possible_id in self.ids:
                possible_id = random.random()
            self.ids.append(possible_id)
            ubicacion = self.chance_ubicaciones_estaciones[
                random.randrange(0, len(self.chance_ubicaciones_estaciones))
            ]
            destino = self.posibles_ubicaciones[
                random.randrange(0, len(self.posibles_ubicaciones))
            ]
            ...
            pasajero_temp = pasajero.Pasajero(
                possible_id, self, ubicacion=ubicacion, destino=destino
            )
            self.schedule.add(pasajero_temp)
            self.grid.place_agent(
                pasajero_temp,
                (
                    pasajero_temp.ubicacion * (15) + random.randrange(0, 10),
                    random.randrange(12, 21),
                ),
            )
